...
- **新機能開発**
  ...
  BizteX Connect は SaaS サービスと SaaS サービスを連携させ、業務効率化を図ることができるサービスです。
  そのため、一般的なシステム開発とは異なり、他社の API 仕様を理解し、それを使って連携機能を実装することが求められます。例えると、プログラミング言語を自作しているような感覚に近いです。
  なので、当然ながら仕様も複雑になりやすく、開発自体も難易度が高いと感じることはありますが、自分自身、タスクの自動化を作るのが好きだったり、アルゴリズムを考えるのが好きだったりするので、このような開発には向いていると実感しています。
  また、今後のプロダクトの拡大に向けて、より良い設計を考えることや、他のチームメンバーとのコミュニケーションを取りながら開発を進めることが求められるため、これまで以上にチームメンバーとのコミュニケーションを大切にし、自分の考えをしっかりと伝えることを心がけています。
  今は、フロントエンド中心で関わらせて頂いていますが、今後はバックエンドも含めて全体の開発を担当できるようになりたいと考えています。
