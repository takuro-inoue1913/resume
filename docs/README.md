...
- フロントエンド： React, TypeScript
- バックエンド： TypeScript, Amazon Amplify, Amazon API Gateway, AWS Lambda
- バージョン管理ツール： GitHub
- スケジュール管理ツール： Jira
...
- **ESG データベースプロダクトでのアンケート機能開発**

  SmartESG では、ESG（環境、社会、ガバナンス）に関する情報を収集し、分析するプラットフォームを開発しています。
  サステナビリティ情報開示では、さまざまな評価機関の質問に回答する関係上、システムではそういった評価機関とのデータ連携が多く、かつ、質問のフォーマットもパターンが多いため、フロント側の UI にしても柔軟性を考慮した設計と実装が求められました。

  主にフロントエンド開発を担当しており、React を用いて、さまざまな要求に対して柔軟に対応できるような UI を作成しています。
  バックエンドは Amazon Amplify でのデプロイ、AWS Lambda と Amazon API Gateway を利用した構成になっているため、要件によっては API の設計や実装にも対応しています。また、db schema の設計や GraphQL の設計も行い、フロントエンドとバックエンドの連携を密に行いながら開発を進めています。

  プロダクトの機能開発が活発的に行われている時期であり、かなりプロダクトが複雑化していきました。また、機能開発が最優先で行われていたためテストが少なく、新機能開発やバグ修正をするにも影響範囲を把握しづらい状況でした。

  この課題に対して、実装や修正する際に仕様に対してプロダクト全体の現状を確認し、影響範囲を把握した上で見積もりを出した上で対応に移りました。また、対応後は全体のテストではなく部分的にユニットテストを個々に実装することで、デグレが起きないような対応を心がけました。
  その結果、他の機能追加に影響されることが少なく、かつ保守性も担保できるような開発が可能になりました。
